...
<session class="find-password-root e-find-password-root">
	<div class="bootstrap-components find-password__text">
		<h1 class="h3-bold find-password__title">비밀번호 재설정</h1>
		...
		<form name="frm" class="find-password__form e-find-password__form" action="${contextPath }/member/findPassword" method="post">
			<input class="form-control" name="email" type="email" placeholder="example@gmail.com" id="userEmail"/>
			<div class="invalid-feedback">이메일 형식에 맞지 않습니다.</div>
			<button type="button" class="btn btn-primary " id="checkEmail">비밀번호 발송</button>
		</form>
	</div>
</session>
...
$("#checkEmail").click(function () {
        ...
        const sendEmailForm = document.forms.frm;
        console.log("sendEmailForm  >>> ", sendEmailForm);
        
        $.ajax({
            type: 'post',
            url: '${contextPath }/member/emailDuplication',
            data: {
                'email': userEmail
            },
            dataType: "text",
